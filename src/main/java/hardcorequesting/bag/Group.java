    ...
    public void draw(GuiQuestBook gui, int x, int y) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (gui.inBounds(xPos, yPos, GuiQuestBook.ITEM_SIZE, GuiQuestBook.ITEM_SIZE, x, y)) {
                if (stack != null) {
                    try {
                        gui.drawMouseOver(stack.getTooltip(Minecraft.getMinecraft().thePlayer, Minecraft.getMinecraft().gameSettings.advancedItemTooltips), x + gui.getLeft(), y + gui.getTop());
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (gui.inBounds(xPos, yPos, GuiQuestBook.ITEM_SIZE, GuiQuestBook.ITEM_SIZE, x, y)) {
                if (gui.getCurrentMode() == EditMode.ITEM) {
                    ItemStack stack = i < items.size() ? items.get(i) : null;
                    int amount;
                    if (stack != null) {
                        stack = stack.copy();
