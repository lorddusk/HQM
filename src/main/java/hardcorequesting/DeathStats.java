...
public class DeathStats implements Serializable {
	...
	private static class DeathStatsBest extends DeathStats {
		private static final String[] colourPrefixes = { GuiColor.YELLOW.toString(), GuiColor.LIGHT_GRAY.toString(),
			...
			for (int i = 0; i < messages.length; i++) {
				...
				if (clientDeathList[0].getDeaths(i) == 0) {
					...
					for (int j = 0; j < clientDeathList.length; j++) {
						...
						messages[i] += colourPrefixes[standing]
								+ Translator.translate("hqm.deathStat." + placePrefixes[standing]);
						messages[i] += GuiColor.WHITE + " " + clientDeathList[j].getName() + ": "
								+ clientDeathList[j].getDeaths(i);
					}
	...
	private static class DeathStatsTotal extends DeathStats {
		...
		public String getDescription(int id) {
			return super.getDescription(id) + "\n\n"
					+ (count[id] == 0 ? GuiColor.RED + Translator.translate("hqm.deathStat.noOneDied")
							: GuiColor.GREEN.toString() + count[id] + " "
									+ Translator.translate("hqm.deathStat.player" + (count[id] == 1 ? "" : "s")) + " "
									+ Translator.translate("hqm.deathStat.diedThisWay"));
		}
	}
	...
	/**
	 * @param clientDeathList
	 *            the clientDeathList to set
	 */
