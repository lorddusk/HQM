...
public abstract class FileHelper {
	...
	public enum SaveResult {
		SUCCESS("Success", "Everything was successfully saved"), BACKUP_FAIL("Backup failure",
				"Couldn't backup the previous saved data. Please fix this and save again."), SAVE_FAIL("Save failure",
						"Couldn't save the data to file. Your previous backup has been saved."), PRE_CRASH_FAILURE(
								"Double save failure",
								"Couldn't save the data to file. And when trying to backup your previously saved data, this didn't work either.");
	...
	/**
	 * Makes sure that the supplied folder is created, along with all its parent
	 * folders. This is to make sure the path actually exists
	 * 
	 * @param dir
	...
	public SaveResult saveData(File file) {
		...
		try {
			...
			if (moveUpFileIndices(file.getAbsolutePath() + PRE_CRASH_SUFFIX, 0)) {
				if (backup(new File(file.getAbsolutePath() + BACKUP_SUFFIX),
						new File(file.getAbsolutePath() + PRE_CRASH_SUFFIX + 0))) {
					success = true;
				}
			}
	...
	public boolean backup(File fileToBackUp) {
		// return backup(fileToBackUp, new File(fileToBackUp.getAbsolutePath() +
		// BACKUP_SUFFIX)); DISABLED TO PREVENT LAGSPIKES
		return true;
	}
