...
public class Team implements Serializable {
	...
	public void receiveAndSyncReputation(Quest quest, List<Quest.ReputationReward> reputationList) {
		for (Quest.ReputationReward reputationReward : reputationList) {
			setReputation(reputationReward.getReputation(),
					getReputation(reputationReward.getReputation()) + reputationReward.getValue());
		}
	...
	public enum LifeSetting implements Serializable {
		SHARE("hqm.team.sharedLives.title", "hqm.team.sharedLives.desc"), INDIVIDUAL("hqm.team.individualLives.title",
				"hqm.team.individualLives.desc");
	...
	public enum RewardSetting implements Serializable {
		ALL("hqm.team.allReward.title", "hqm.team.allReward.desc"), ANY("hqm.team.anyReward.title",
				"hqm.team.anyReward.desc"), RANDOM("hqm.team.randomReward.title", "hqm.team.randomReward.desc");
	...
	public enum UpdateType {
		ALL, ONLY_MEMBERS, ONLY_INVITES, ONLY_OWNER
	}
	...
	public enum RefreshType {
		FULL, LIVES, REPUTATION_RECEIVED
	}
	...
	public void deleteTeam() {
		...
		for (String username : FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager()
				.getAllUsernames()) {
	...
	private enum TeamAction {
		CREATE, INVITE, ACCEPT, DECLINE, KICK, LEAVE, DISBAND, NEXT_LIFE_SETTING, NEXT_REWARD_SETTING
	}
	...
	// slightly ugly but there's no real way of getting hold of the interface,
	// this works perfectly fine
	public static ErrorMessage latestError;

	public enum ErrorMessage {
		INVALID_PLAYER("hqm.team.invalidPlayer.title", "hqm.team.invalidPlayer.desc"), IN_PARTY(
				"hqm.team.playerInParty.title",
				"hqm.team.playerInParty.desc"), USED_NAME("hqm.team.usedTeamName.title", "hqm.team.usedTeamName.desc");
	...
	private static void handleServerPacket(EntityPlayer player, DataReader dr) {
							...
							for (int i = 0; i < inviteTeam.questData.size(); i++) {
								...
								if (questData != null) {
									...
									for (int j = 0; j < questData.reward.length; j++) {
										if (j == id) {
											questData.reward[j] = joinData.reward[0]; // you
																						// keep
																						// your
																						// reward
										} else if (j < id) {
		...
		case NEXT_REWARD_SETTING:
			if (!team.isSingle() && team.isOwner(playerName)) {
				team.rewardSetting = RewardSetting.values()[(team.rewardSetting.ordinal() + 1)
						% RewardSetting.values().length];
