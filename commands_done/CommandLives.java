...
public class CommandLives extends CommandBase {
    ...
    public void handleCommand(ICommandSender sender, String[] arguments) throws CommandException {
        ...
        if (arguments.length == 0 && sender instanceof PlayerEntity)
        ...
        if (arguments.length == 1) {
            if (arguments[0].matches(REMOVE)) {
                if (sender instanceof PlayerEntity)
                    removeLives((PlayerEntity) sender, 1);
            } else if (arguments[0].matches(ADD)) {
                if (sender instanceof PlayerEntity)
                    addLives((PlayerEntity) sender, 1);
            } else {
            ...
            if (arguments[0].matches(REMOVE)) {
                try {
                    ...
                    if (sender instanceof PlayerEntity)
                        removeLives((PlayerEntity) sender, amount);
                } catch (Exception e) {
            ...
            //add amount own
                ...
                try {
                    ...
                    if (amount < 0)
                        throw new WrongUsageException("hqm.message.positiveNumbers");
                    if (sender instanceof PlayerEntity)
                        addLives((PlayerEntity) sender, amount);
                } catch (Exception e) {
                    throw new WrongUsageException("hqm.message.positiveNumbers");
                }
            }
    ...
    private void removeLives(PlayerEntity player, int amount) {
    ...
    private void removeLivesFrom(ICommandSender sender, String playerName, int amount) {
        PlayerEntity player = HardcoreQuesting.getServer().getPlayerManager().getPlayerByUsername(playerName);
        
        QuestingData.getQuestingData(player).removeLives((ServerPlayerEntity) sender, amount);
    ...
    private void addLives(PlayerEntity player, int amount) {
