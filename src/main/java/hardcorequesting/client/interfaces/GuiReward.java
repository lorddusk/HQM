    ...
    public void drawScreen(int mX0, int mY0, float f) {
        ...
        for (Reward reward : rewards) {
            try {
                ...
                itemRenderer.renderItemOverlayIntoGUI(fontRendererObj, reward.stack, (reward.x + left + 1), (reward.y + top + 1), "");
            ...
            if (inBounds(reward.x, reward.y, ITEM_SIZE, ITEM_SIZE, mX, mY)) {
                try {
                    if (GuiScreen.isShiftKeyDown()) {
                        drawMouseOver(reward.stack.getTooltip(Minecraft.getMinecraft().thePlayer, Minecraft.getMinecraft().gameSettings.advancedItemTooltips), mX0, mY0);
                    } else {
                        List<String> str = new ArrayList<String>();
                        try {
                            List info = reward.stack.getTooltip(Minecraft.getMinecraft().thePlayer, Minecraft.getMinecraft().gameSettings.advancedItemTooltips);
                            if (info.size() > 0) {
