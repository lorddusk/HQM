                ...
                if (!world.isRemote) {
                    ...
                    if (te != null && te instanceof TileEntityBarrel) {
                        if (((TileEntityBarrel) te).getCurrentTask() != null && te.getBlockMetadata() == 1) {
                            line = "The QDS is currently bound to '" + Quest.getQuest(((TileEntityBarrel) te).selectedQuest).getName() + "'";
                            player.addChatComponentMessage(Util.getChatComponent(line, EnumChatFormatting.WHITE));
                        } else {
                            line = "You currently have not bound a quest to the QDS";
                            player.addChatComponentMessage(Util.getChatComponent(line, EnumChatFormatting.WHITE));
                        }
                    }
                }
                ...
                if (!world.isRemote) {
                    ...
                    if (te != null && te instanceof TileEntityBarrel) {
                        ...
                        if (((TileEntityBarrel) te).getCurrentTask() != null) {
                            line = "You bound '" + Quest.getQuest(((TileEntityBarrel) te).selectedQuest).getName() + "' to the QDS";
                            player.addChatComponentMessage(Util.getChatComponent(line, EnumChatFormatting.WHITE));
                        } else {
                            line = "You currently have not selected any tasks";
                            player.addChatComponentMessage(Util.getChatComponent(line, EnumChatFormatting.WHITE));
                        }
