...
public class PatchedNbtOps extends NbtOps {
    ...
    public Tag createList(Stream<Tag> stream) {
        ...
        if (elements.isEmpty())
            ...
            return new ByteArrayTag(elements.stream().map(tag -> ((NumericTag) tag).getAsByte()).toList());
        } else if (elements.stream().allMatch(this::canBeInIntArray)) {
            return new IntArrayTag(elements.stream().map(tag -> ((NumericTag) tag).getAsInt()).toList());
        } else if (elements.stream().allMatch(this::canBeInLongArray)) {
            return new LongArrayTag(elements.stream().map(tag -> ((NumericTag) tag).getAsLong()).toList());
        } else {
