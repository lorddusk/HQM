...
import hardcorequesting.quests.QuestAdapter;
...
public class CommandHandler extends CommandBase {
    ...
    static {
        GSON = new GsonBuilder().registerTypeAdapter(QuestSet.class, QuestAdapter.QUEST_SET_ADAPTER).setPrettyPrinting()
...
//		}
        ...
        if (arguments[0].matches("help") && isPlayerOp(sender)) {
            ...
            sendChat(sender, "- load <filename> : Loads given quest line into HQM.");
            sendChat(sender, "- load all : Loads all files into HQM.");
            sendChat(sender, "- save <Quest Page> [filename] : Saves give quest page to JSON, filename defaults to page name.");
            sendChat(sender, "- save all : Saves all files to JSON");
            return;
        ...
        if (arguments[0].matches("load")) {
            if (arguments[1].matches("all")) {
                ...
                CommandLoad.load(sender, getFile(file));
            }
            Quest.FILE_HELPER.saveData(null);
        }

        if (arguments[0].matches("save")) {
            if (arguments.length == 2 && arguments[1].equals("all")) {
                for (QuestSet set : Quest.getQuestSets()) {
                    try {
                        CommandSave.save(sender, set, set.getName());
                ...
                for (QuestSet set : Quest.getQuestSets()) {
                    ...
                    if (name.length < arguments.length && stringsMatch(name, arguments)) {
                        ...
                        CommandSave.save(sender, set, fileName);
                        return;
                ...
                throw new CommandException("Quest page %s not found", arg);
            }
        }
